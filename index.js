...
function createDictionary(entries) {
  ...
  entries.forEach(([value, key]) => lib.set(value, key));
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) => {
    if (!dictionary.has(key)) {
      dictionary.set(key, value); // Якщо ключ є унікальним, додаємо його до словника за допомогою методу set та збільшимо added на 1
      added++;
    } else {
      rejected++;
    }
...
function convertSetToDictionary(set) {
  ...
  for (const element of set) {
    map.set(element, element.charCodeAt(0));
  }
